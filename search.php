...
if(isset($_GET['q'])){
	$cond = array(
			'titre' => '%'.strtolower($_GET['q']).'%',
			'description' => '%'.strtolower($_GET['q']).'%'
		);
}

//pagination

	$nbr = $DB->query("SELECT count(*) as nbr FROM posts WHERE LOWER(titre) LIKE :titre OR LOWER(description) LIKE :description" ,$cond);
...
$sql ='SELECT posts.id,posts.titre,posts.description,posts.created_at,posts.image,posts.category_id,users.username,categories.name FROM posts
		...
		WHERE LOWER(titre) LIKE :titre OR LOWER(description) LIKE :description
		...
		<div id="content">
			<?php if (empty($posts)): ?>
				<article  class="clearfix">
					<h2>Résultats :</h2>
					<p>Aucun résultat pour <strong><?php echo $_GET['q'] ?></strong></p>
				</article>
			<?php else : ?>
